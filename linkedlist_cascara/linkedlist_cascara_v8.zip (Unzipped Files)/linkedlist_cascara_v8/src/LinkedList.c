...
static Node* getNode(LinkedList* this, int nodeIndex)
{
    ...
    {
        pNodo = this->pFirstNode; // Apunto a la direccion de memoria del primer nodo

        while(nodeIndex > 0)
        {
            pNodo = pNodo->pNextNode; // De ser mayor a 0, itero hasta alcanzar el indice deseado y rescato la direccion de memoria del nodo siguiente
            nodeIndex--;
        }
    }
    return pNodo;
...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if(this != NULL && nodeIndex >= 0 && nodeIndex <= tam)
    {
        ...
        if(nuevoNodo != NULL)
        {
            nuevoNodo->pElement = pElement; //Cargo los datos que recibi por parametro en el nuevo nodo

            if(nodeIndex == 0)
            {
                nuevoNodo->pNextNode = this->pFirstNode;
                this->pFirstNode = nuevoNodo; //Engancho a la locomotora el nodo nuevo
            }
            else
            {
                nuevoNodo->pNextNode = getNode(this,nodeIndex); // Lo engancho en la posicion donde necessito
                (getNode(this,nodeIndex-1))->pNextNode = nuevoNodo; //Hago que el nodo anterior apunte a la posicion nueva
            }
            this->size++; //Actualizo el tamaño de la locomotora
            returnAux = 0;
        }
    }
    return returnAux;
